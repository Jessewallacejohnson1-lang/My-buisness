...
## Decided 2026-09-24 — Create removed, posting paused

Jesse paused posting. Nothing in the app opens a composer any more, and there is no
replacement entry point. The tab bar is back to **four even slots built from
`Tab.allCases`**, the shape it had before the Alta pass (`9544115^`); `TabBarTests` pins
...
- **Deleted:** `Features/Add/` (`AddView`, `AddFormView`, `AddModel`, `AddKind`),
  ...
  `TodayFeedView.onCompose` and its plus, the day sheet's "Add to today" button with its
  `.callToAction` anchor, `cta` fixture state and `cta*` metrics, `YourDayRailCopy.addTile`,
  and the `-open-speeddial` / `-speeddial-loop` flags.
- **Back-end writes left dead, deliberately not touched.** Their only callers were the
  deleted composer: `CommunityAPI.addEvent`, `.addTrail`, `.submitClub`;
  `Storage.uploadEventImage`; `NewEventInput` / `NewTrailInput` / `ClubInput`;
  `KnownVenues.suggestions`; `GooglePlacesService.autocomplete`. No Supabase, RLS,
  migration or edge-function change. When posting returns, these are the starting point.
- **Carried through the `chore/instruction-system` merge.** That branch moved the
  AGENTS.md prose into `docs/rules/architecture.md`, `docs/rules/design.md` and
  `docs/rules/map.md`; these edits landed there when `origin/main` was merged into
  `feat/remove-create`, and AGENTS.md is regenerated by `scripts/bp_rules.py build`.
