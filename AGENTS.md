...
Both AGENTS.md and memory load every session, so a lesson logged once is applied forever. Don't wait to be asked — logging the lesson is part of finishing the task.
...
Codex each spawn their own Metro on a different port (8081, 8082, 8090…), the servers
...
1. **One server, Codex-controlled, kept alive.** Don't trust an agent tab's volatile
...
files** (no two editors per file); **idle ≠ done** (resuming a tab after Codex edits
...
- **Any bottom sheet/day-sheet/detail sheet uses `components/ui/bottom-sheet.tsx`
  ...
  (`ExpandedWeek` pinch-zoom, `TownMap` card, `AccountMenu` glass showcase) can't adopt it
  but follow the same rule: a visible `ink2` close + a tap/drag exit that works with a mouse,
  never gesture-only (pinch/swipe don't exist on the web build). **Web gotcha:** on the web
  build `react-native-web` renders `Pressable` as a real `<button>`, so a full-screen
  backdrop `Pressable` must be a **sibling behind** the overlay content (absolute-fill),
  **never wrap it** — wrapping nests the content's buttons inside the backdrop button and
  React logs an invalid-DOM/hydration error. Center the content with a plain
  `pointerEvents="box-none"` `View` so empty-margin taps fall through to the backdrop (this
  is exactly how `bottom-sheet.tsx` and `AccountMenu` are structured).
...
- **Backend:** **Supabase** — auth (email+password, confirmation ON) + Postgres (all tables
  have RLS) + **Edge Functions** (`supabase/functions/moderate-post` runs Codex moderation).
...
Installed at `~/.Codex/plugins/cache/Codex-plugins-official/`:
...
- 168 slash commands → `~/.Codex/commands/`
- 108 specialized agents → `~/.Codex/agents/`
- Hooks registered in `~/.Codex/settings.json` (PreToolUse/PostToolUse/Stop)
...
3. **Run the app and screenshot at the reference's viewport.** This app is
   ...
   `preview_screenshot`); see `.Codex/TOOLKIT.md`. Capture it yourself — never
