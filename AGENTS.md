...
- **Today:** `-briefing-preview [-briefing-state sample|one_event|zero_events|voted|none|degraded]` — the payload still loads, but with the registry empty this now renders the top bar over an empty feed; `-feed-scrolled` starts it 420pt down. `-briefing-gallery` / `-gallery-state` died with the module galleries.
