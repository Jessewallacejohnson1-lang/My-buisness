...
> **Locations (June 2026 reorg):** `brand`, `design-system`, `slides`,
> `banner-design` now live under `../../brand-visual/`; `ui-styling` lives under
> `../../ui-layout/`. `logo-design`, `cip-design`, `icon-design`, and social
> photos remain built into this `design/` skill (`scripts/` + `references/`).
...
**Brand:**
```bash
node .claude/skills/brand-visual/brand/scripts/inject-brand-context.cjs
node .claude/skills/brand-visual/brand/scripts/validate-asset.cjs <path>
```

**Tokens:**
```bash
node .claude/skills/brand-visual/design-system/scripts/generate-tokens.cjs -c tokens.json
node .claude/skills/brand-visual/design-system/scripts/validate-tokens.cjs -d src/
```
