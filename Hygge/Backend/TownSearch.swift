...
enum TownSearch {
    ...
    static func score(_ query: String, name: String, extra: [String] = [], aliases: [String] = []) -> Double? {
        ...
        if nn.contains(q) { return 800 }

        for a in aliases {
            let an = norm(a)
            if an.isEmpty { continue }
            if an == q { return 780 }
            if an.contains(q) || q.contains(an) { return 720 }
        }
    ...
    private static func bestTokenScore(_ q: String, _ tokens: [String]) -> Double? {
        ...
        for t in tokens {
            ...
            } else if q.count >= 3 && t.contains(q) {
                s = 60
            } else {
                // Only spend Levenshtein on tokens long enough to typo meaningfully.
    ...
    static func expandedTerms(_ query: String) -> [String] {
        ...
        for interest in Interests.all {
            ...
            let hitKw = interest.keywords.contains { kw in
                let k = norm(kw); return !k.isEmpty && (k.contains(q) || q.contains(k))
            }
            if hitLabel || hitKw {
        ...
        for spot in MapSpots.all {
            let hit = norm(spot.name).contains(q)
                || spot.keywords.contains { let k = norm($0); return !k.isEmpty && (k.contains(q) || q.contains(k)) }
            if hit {
