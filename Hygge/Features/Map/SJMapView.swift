...
struct SJMapView: View {
    ...
    var body: some View {
        ZStack(alignment: .top) {
            mapLayer
            // Behind the sheet + chrome, over the map: a soft fade so the live map's
            // labels/roads/pins near the bottom don't read through the translucent
            // (Liquid Glass) app tab bar or in the strip below it. It sits under the
            // MapSheet (so the sheet's white is never tinted) and under the app tab bar.
            mapBottomFade
            topChrome
            floatingControls
            MapSheet(
    ...
    /// A soft fade over the map's bottom band. Every other tab sits over the calm
    /// canvas, but the map is a busy backdrop — without this, map detail bleeds
    /// through the floating Liquid-Glass tab bar and shows in the strip beneath it.
    /// The fade dissolves that band into the basemap's own cream (`Hue.mapLand`), so
    /// it reads as the map quietly lightening — a fade, not a hard seal — keeping the
    /// full-bleed map. Non-interactive (taps pass through to the map).
    private var mapBottomFade: some View {
        // A frosted dissolve, not a flat wash. Two layers on one top→bottom mask:
        //   1. `.ultraThinMaterial` — the SAME material language as the Liquid-Glass
        //      tab bar above it — blurs the live map (labels/roads/pins) into softness
        //      color-agnostically, so it reads right over land, water AND parks (a flat
        //      cream fill only matched the cream land).
        //   2. a basemap-cream wash (`Hue.mapLand`) — a bare material goes cold-grey
        //      over the warm map, so this keeps the frost warm and seals the very
        //      bottom (home-indicator strip) to calm cream.
        // The shared gradient makes it a dissolve (soft, invisible top → present at the
        // bottom), never a bar. Non-interactive; taps pass through to the map.
        ZStack {
            Rectangle()
            ...
            LinearGradient(
                stops: [
                    .init(color: Hue.mapLand.opacity(0),    location: 0.0),
                    .init(color: Hue.mapLand.opacity(0.32), location: 0.55),
                    .init(color: Hue.mapLand.opacity(0.82), location: 1.0),
                ],
                startPoint: .top, endPoint: .bottom
            )
        }
    ...
    private var townPill: some View {
        ...
        .padding(.vertical, 8)
        .glassEffect(.regular, in: Capsule())   // same Liquid Glass as the chrome + tab bar
    ...
    /// The shared floating chrome bubble — real Liquid Glass (the SAME material as the
    /// tab bar, so the map's controls read as one system), a legible ink icon, and a
    /// coral tint + ring when active. Glass carries its own floating shadow, so no
    /// separate `mapFloatShadow`. The press-pop comes from the button's PressableStyle.
    private func chromeCircle(icon: String, active: Bool = false) -> some View {
        Image(systemName: icon)
            ...
            .glassEffect(active ? .regular.tint(Hue.accent.opacity(0.16)) : .regular, in: Circle())
            .overlay {
                if active { Circle().stroke(Hue.accent.opacity(0.55), lineWidth: 1.5) }
            }
    }
