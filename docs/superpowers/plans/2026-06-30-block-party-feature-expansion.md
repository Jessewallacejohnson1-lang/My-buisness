# Block Party Feature Expansion Implementation Plan
...
- **Verify one step at a time; commit after each task** in the `BlockParty` repo (`/Users/owner/Documents/Block Party`).
...
**New backend:**
- `BlockParty/Backend/Storage.swift` — upload image Data → `event-images` bucket → public URL.
- `BlockParty/Backend/Moderation.swift` — invoke `moderate-post` Edge Function → `(ok, reason)`.
- `BlockParty/Backend/Interests.swift` — INTERESTS taxonomy, `matchesInterests`, UserDefaults get/set + onboarded flag.
- `BlockParty/Backend/Reminders.swift` — request permission + schedule/cancel one local notification per event.

**New features:**
- `BlockParty/Features/Add/AddModel.swift` — form state + submit pipeline (upload → moderate → insert).
- `BlockParty/Features/Add/AddFormView.swift` — the event/club/trail form UI.
- `BlockParty/Features/Onboarding/OnboardingView.swift` — hello → interests → done.
- `BlockParty/Features/Components/InviteCard.swift` — rendered invite card + ShareLink.

**Modified:**
- `BlockParty/Features/Add/AddView.swift` — kind chooser navigates to `AddFormView(kind:)`.
- `BlockParty/App/RootView.swift` — gate: signed-in + not onboarded → `OnboardingView`.
- `BlockParty/Features/Home/HomeView.swift` + `HomeModel.swift` — "Suggested for you" via interests; "Add an event" opens Add.
- `BlockParty/Features/Activities/ActivitiesView.swift` + `ActivitiesModel.swift` — interest-ranked ordering + invite button.
- `BlockParty/Features/Components/EventRow.swift` — add "Invite a neighbor" + "Remind me".
- `BlockParty/Backend/CommunityAPI.swift` — no change to signatures; reused as-is.
...
**Files:**
- Create: `BlockParty/Backend/Storage.swift`
...
- [ ] **Step 3: Commit** — `git add BlockParty/Backend/Storage.swift && git commit -m "feat(backend): event-images storage upload"`
...
**Files:**
- Create: `BlockParty/Backend/Moderation.swift`
...
**Files:**
- Create: `BlockParty/Features/Add/AddModel.swift`
...
**Files:**
- Create: `BlockParty/Features/Add/AddFormView.swift`
- Modify: `BlockParty/Features/Add/AddView.swift` (kind cards navigate to the form)
...
**Files:**
- Create: `BlockParty/Backend/Interests.swift`
...
**Files:**
- Create: `BlockParty/Features/Onboarding/OnboardingView.swift`
...
**Files:**
- Modify: `BlockParty/Features/Activities/ActivitiesView.swift` (+`ActivitiesModel.swift`), `BlockParty/Features/Home/HomeView.swift` (+`HomeModel.swift`)
...
**Files:**
- Create: `BlockParty/Features/Components/InviteCard.swift`
- Modify: `BlockParty/Features/Components/EventRow.swift` (add invite affordance)
...
- `ShareLink` on an "Invite a neighbor" button in `EventRow`, sharing the rendered image + a plain-text line: `"Come to \(title) with me — \(prettyDate) at \(location). (via Block Party)"`. No deep link/app graph; just the OS share sheet.
...
**Files:**
- Create: `BlockParty/Backend/Reminders.swift`
- Modify: `BlockParty/Features/Components/EventRow.swift` (a "Remind me" toggle)
