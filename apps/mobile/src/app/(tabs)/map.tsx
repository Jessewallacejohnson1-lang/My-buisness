...
import { minutesOf, nowMinutes } from '../../lib/time'
import { C, F, HAIRLINE, CARD_SHADOW } from '../../theme'
import { PlusIcon, PinIcon, TrailIcon, EventIcon, CloseIcon } from '../../components/icons'
...
// two hours — not just "some time today." Untimed (all-day) events never glow.
            ...
            {liveCount > 0 && (
              <>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5 }}>
                  <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: C.moss700 }} />
                  <Text style={{ fontFamily: F.sansSemi, fontSize: 13, color: C.moss700 }}>
                    {liveCount} now
                  </Text>
                </View>
...
function EventPin({ live, selected }: { live: boolean; selected: boolean }) {
  ...
  const bg = selected ? C.ink : live ? C.moss700 : C.paper
  ...
  return (
    <View style={{ width: 56, height: 56, alignItems: 'center', justifyContent: 'center' }}>
      {live && (
        <Animated.View
          style={[
            { position: 'absolute', width: 36, height: 36, borderRadius: 18, backgroundColor: C.moss700 },
  ...
  return (
    <View style={{ gap: 6, paddingRight: 24 }}>
      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 }}>
        <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: live ? C.moss700 : C.ink3 }} />
        <Text style={{ fontFamily: F.mono, fontSize: 10, color: live ? C.moss700 : C.ink3, letterSpacing: 1.2, textTransform: 'uppercase' }}>
