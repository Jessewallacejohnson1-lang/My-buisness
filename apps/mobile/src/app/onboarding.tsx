...
export default function Onboarding() {
  ...
  const [editing, setEditing] = useState(false) // re-entry from "Edit interests" vs first run
  ...
  // Editing existing interests: back out without touching anything.
  const cancelEdit = () => { if (router.canGoBack()) router.back(); else router.replace('/(tabs)') }
  // Wrong account: sign out — the auth gate then routes to /login.
  const leaveAccount = async () => { await signOut() }
      ...
      <ScrollView contentContainerStyle={{ flexGrow: 1, paddingHorizontal: 24, paddingTop: 28, paddingBottom: 36 }}>
        {step === 'hello' && (
          <View style={{ flex: 1, justifyContent: 'center' }}>
            <Text style={{ fontWeight: F.sansMed, fontSize: 12, color: C.ink3, letterSpacing: 1.8, textTransform: 'uppercase', marginBottom: 10 }}>Welcome</Text>
            <Text style={{ fontWeight: F.display, fontSize: 34, color: C.ink, lineHeight: 40, marginBottom: 14 }}>
              One calm place for everything happening in St. Joe.
            </Text>
            <Text style={{ fontWeight: F.sans, fontSize: 16, color: C.ink2, lineHeight: 24, marginBottom: 36 }}>
            ...
            <Pressable onPress={() => { Haptics.selectionAsync(); setStep('interests') }}
              ...
              <Text style={{ fontWeight: F.sansSemi, fontSize: 16, color: C.paper }}>Get started</Text>
            </Pressable>
            <Pressable onPress={skip} style={{ paddingVertical: 14, alignItems: 'center' }}>
              <Text style={{ fontWeight: F.sansMed, fontSize: 14, color: C.ink3 }}>Skip for now</Text>
            </Pressable>
            <Pressable onPress={leaveAccount} hitSlop={6} style={{ paddingVertical: 6, alignItems: 'center' }}>
              <Text style={{ fontWeight: F.sansMed, fontSize: 13, color: C.ink2 }}>Not you? Sign out</Text>
            </Pressable>
          </View>
        )}
        ...
        {step === 'interests' && (
          <View style={{ flex: 1 }}>
            <Text style={{ fontWeight: F.display, fontSize: 28, color: C.ink, marginBottom: 6 }}>What are you into?</Text>
            <Text style={{ fontWeight: F.sans, fontSize: 15, color: C.ink2, marginBottom: 22 }}>Pick a few — we'll suggest matches.</Text>
              ...
              {INTERESTS.map((it) => {
                ...
                return (
                  <Pressable key={it.id} onPress={() => toggle(it.id)}
                    ...
                    <Text style={{ fontWeight: F.sansMed, fontSize: 14, color: on ? C.paper : C.ink2 }}>{it.label}</Text>
                  </Pressable>
                )
              })}
            </View>
            ...
            <Pressable onPress={() => { Haptics.selectionAsync(); setStep('suggest') }} disabled={selected.length === 0}
              ...
              <Text style={{ fontWeight: F.sansSemi, fontSize: 16, color: selected.length === 0 ? C.ink3 : C.paper }}>Continue</Text>
            </Pressable>
            <Pressable onPress={editing ? cancelEdit : skip} style={{ paddingVertical: 14, alignItems: 'center' }}>
              <Text style={{ fontWeight: F.sansMed, fontSize: 14, color: C.ink3 }}>{editing ? 'Cancel' : 'Skip for now'}</Text>
            </Pressable>
          </View>
        )}
        ...
        {step === 'suggest' && (
          <View style={{ flex: 1 }}>
            <Text style={{ fontWeight: F.display, fontSize: 28, color: C.ink, marginBottom: 6 }}>A few to check out</Text>
            <Text style={{ fontWeight: F.sans, fontSize: 15, color: C.ink2, marginBottom: 22 }}>Based on what you picked.</Text>

            {nothingMatched ? (
              <View style={{ borderRadius: 16, borderWidth: 1.5, borderStyle: 'dashed', borderColor: 'rgba(0,0,0,0.13)', padding: 20 }}>
                <Text style={{ fontWeight: F.sans, fontSize: 14, color: C.ink2, lineHeight: 21 }}>
            ...
            <Pressable onPress={finish}
              ...
              <Text style={{ fontWeight: F.sansSemi, fontSize: 16, color: C.paper }}>Take me in</Text>
            </Pressable>
          </View>
        )}
      </ScrollView>
